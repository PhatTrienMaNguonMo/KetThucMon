<?php
	if(isset($_POST["submit_name"])){//lấy dữ liệu từ form gán vào biến
		...
		$soluong = $_POST["soluong"];
	      ...
	      require("Connect.php");//Kết nối CSDL DB2
			if($conn)
				{   
				    $SQL1= "INSERT INTO user1.MATHANG values('$ma_mh','$ma_ncc','$ten_mh','$soluongnhap','$giaban','$chatlieu')";//Kết nối câu truy vấn
		$stmt = db2_prepare($conn, $SQL1);
		 if ($stmt) {		
								  $result = db2_execute($stmt);
						 			if ($result) 
                               			
		header("location:admin.php?page=Mathang");//chuyển tiếp tới Mặt Hàng
	}
	}
}
